...
df <- read.csv('R-files-and-dataset-for-Module-1/jan17Items.csv'        # You can also enter the full path if the file is not in the working directory
...
dfw <- read.csv('R-files-and-dataset-for-Module-1/jan17Weather.csv'
                , sep = '\t'
                , stringsAsFactors = F)
...
write.csv(dfw
          , 'R-files-and-dataset-for-Module-1/weather2.csv'      # If you want to save to a location not in your working directly, include the full path.
          , row.names = F)      # If this is isn't set to FALSE, it will include a column with the row names.

w2<-read.csv("R-files-and-dataset-for-Module-1/weather2.csv", 
             sep = ','
         , stringsAsFactors = F
         , header=T)
